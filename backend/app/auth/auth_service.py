...
class AuthService:
    ...
    def register_user(self, email: str, password: str, full_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(auth_response, 'error') and auth_response.error:
                logger.error(f"Supabase Auth error: {auth_response.error}")
                error_message = str(auth_response.error.message) if hasattr(auth_response.error, 'message') else str(auth_response.error)
    ...
    def invite_user(self, email: str, redirect_to: str = None) -> Dict[str, Any]:
        """Admin invite a new user via email (requires service role)."""
        try:
            # This requires service role key - make sure we're using it
            if not redirect_to:
                redirect_to = f"{request.host_url}auth/confirm"
            
            # Use the admin.invite_user_by_email method
            ...
            if result.error:
                logger.error(f"Admin invite failed: {result.error.message}")
                return {
                    'success': False,
